...
profile, fp16, max_len 192) — the settled output of flag-visible teacher →
...
```json
{ "query": "6es7 522-1bl01", "segment": "A_identifier", "max_len": 192,
...
* `max_len` is a free serve-time dial (quality is flat 128→256), clamped to
  `[8, CE_MAX_LEN]`, so an A/B can trade window depth against width with no
  redeploy.
