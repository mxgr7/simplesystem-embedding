...
> **TEST_PROFILE_18 conditional flatten (`Test18FilterConsolidator`, flag `s2ng-article-search.test18.denormalized-scoping-filters`, default off).** When the flag is on, the structural **Offers/Prices context** clauses above are emitted as a flat `terms` on the denormalized `catalogVersionIds` / `priceKeys` (§2.1.2) instead of `nested(offers|prices, …)` — **but only when no other clause targets the same nested path**. If any provider contributes an `offers`/`prices` nested clause (manufacturer, category, eClass, delivery-time, **price range**, …), the structural scoping for that path stays `nested` so the §2.2.2 same-offer / same-price correlation is preserved (the consolidator merges sibling same-path nesteds into one `nested(bool{filter:[…]})`; flattening would break that conjunction). It is a **whole-index swap, not a per-doc `OR` fallback** — an `OR(flat, nested)` would still evaluate the nested join and save nothing — so the flag asserts the target index is fully backfilled with the §2.1.2 fields (true by construction for a fresh index cut from `target_mapping.json`). Off ⇒ legacy all-nested behaviour, safe for pre-denormalization indices.
...
- Top-level multi-valued scalars `catalogVersionIds` and `priceKeys` (keyword) — denormalizations of the offers/prices scoping, §2.1.2.
...
| `catalogVersionIds` | `keyword` (multi) | Parent-level **union** of every `offers[].catalogVersionIds`. Lets TEST_PROFILE_18 apply the catalog-version scoping as a flat `terms` instead of a `nested(offers, …)` block-join (profiled as the dominant filtered-kNN cost). Exact for the standalone catalog-version predicate — a lone multi-valued field has no same-offer correlation partner (§2.2.2 trivially preserved). Consumed by the conditional flatten in §1.3.1 / `Test18FilterConsolidator`. |
| `priceKeys` | `keyword` (multi) | Parent-level set of `"{priceListId}|{currency}"`, **one token per `prices[]` entry**. Lets the price-list × currency scoping ride as a flat `terms` while preserving the **same-price equality correlation** (§2.2.2) *exactly* — a token exists iff that pair occurred in one price doc. Does **not** encode the price *amount*; a price *range* predicate is a correlated range that cannot be tokenized and stays `nested(prices)` (see §2.1.4). |
...
- **Derive the denormalized scopes from the same nested data.** When building the article doc, set `catalogVersionIds` = sorted distinct union of every `offers[].catalogVersionIds`, and `priceKeys` = sorted distinct set of `"{priceListId}|{currency}"` over every `prices[]` entry. Pure functions of the offers/prices already in the doc — no extra source reads. Emit them even when empty (an empty set is "no offers/prices", distinct from a legacy doc that predates the fields and has them absent). Backfilling existing docs needs a reindex/replay (additive mapping ⇒ no breaking change, but the fields are null until a doc is re-emitted); a server-side `_reindex` with a painless derive script populates them without re-running the embed pass.
...
Selected rows; full grid in `reports/hnsw_eval/bench_recall_at_10_es.json`.
...
#### 2.1.4.2 Sweep 2 — precision × `num_candidates`, fixed (m=16, ef_construction=100)

Full results in `reports/hnsw_eval/bench_recall_at_10_precision_m16_efc100.json`.
...
| `acl-top+mfr-small` | 0.00% | any | ~1 ms | ~1 ms | 1.0000 |
...
> **Status (denormalized scoping, §2.1.2 `priceKeys`/`catalogVersionIds`):** the `priceKeys` denormalization eliminates `nested(prices)` **only for the equality-correlated price-list × currency scoping** — that scoping is now a flat `terms`, and a profiled comparison on the full v3-shape index showed the structural filter cost collapse ~30× (nested block-join `took≈2700 ms` → flat-terms `took≈90 ms`, identical hits and identical kNN `vector_operations`). It does **not** retire the `num_candidates=5000` driver: the worst-case is the price *range* (`minAmount/maxAmount`), a *correlated range* that cannot be tokenized into `priceKeys` and therefore **stays `nested(prices)`** (the consumer keeps it nested to preserve §2.2.2 same-price correlation — see §1.3.1). So §2.1.4.3's price-range regime — the actual reason for 5000 — is unchanged by `priceKeys`. Fully retiring the 5000 driver still requires the separate top-level **min/max-price materialization** suggested above (not yet implemented); the catalog-version / price-list joins it does flatten were never the recall-limiting predicate.
...
| regime | sel% | numC | rec@10 v2 (int8) | rec@10 v3 (fp32) | Δ |
|:---|---:|---:|---:|---:|---:|
...
| regime | numC | p50 v2 (ms) | p50 v3 (ms) |
|:---|---:|---:|---:|
...
#### 2.1.4.5 Sweep 5 — STANDARD vs. TP18 head-to-head latency

Full results in `reports/hnsw_eval_full/bench_profiles_latency_c4.json`. Sweep 4 characterised the TP18 vector retriever's recall/latency in isolation; sweep 5 measures how STANDARD's BM25 profile compares to TP18's three routes (lexical-only, vector-only, RRF-hybrid) under the same filter regimes. All four profile families hit v3 directly so the comparison isolates query shape from infrastructure (same shards, same node, same page cache). STANDARD reproduces §1.2's three-sub-query body verbatim (offer cross-fields multi_match + identifier matches, syntheticKeywords, pre/post filter split, `search_type=dfs_query_then_fetch`); the customer-artno sub-query is dropped (no customer context in synthetic bench). TP18 routes match §2.2.1 (pruned offer bool), §2.2.2 (top-level `knn` block), and §2.2.5 (app-side RRF — parallel legs + phase-2 page fetch); leg depth `numc ∈ {1000, 5000}` for vector and hybrid. 1k queries per cell, concurrency 4 (see saturation note below). Built by `scripts/bench_profiles_latency.py`; queries are the same 1k from §2.1.4.1.

p50 latency (ms) — concurrency 4, ms:

| regime | STANDARD | tp18-lex | vec@1000 | vec@5000 | hyb@1000 | hyb@5000 |
|:---|---:|---:|---:|---:|---:|---:|
| `unfiltered` | 11 | 3 | 45 | 191 | 67 | 254 |
| `acl-top` | 10 | 4 | 172 | 462 | 192 | 533 |
| `acl-top+cat-top` | 6 | 4 | 92 | 117 | 111 | 182 |
| `acl-top+mfr-mid` | 7 | 3 | 90 | 112 | 110 | 179 |
| `acl-top+price-50-200` | 9 | 3 | 179 | 179 | 200 | 253 |

p99 latency (ms) — concurrency 4, ms:

| regime | STANDARD | tp18-lex | vec@1000 | vec@5000 | hyb@1000 | hyb@5000 |
|:---|---:|---:|---:|---:|---:|---:|
| `unfiltered` | 30 | 10 | 61 | 237 | 100 | 1374 |
| `acl-top` | 23 | 6 | 209 | 527 | 230 | 611 |
| `acl-top+cat-top` | 23 | 9 | 107 | 136 | 145 | 230 |
| `acl-top+mfr-mid` | 23 | 6 | 108 | 132 | 129 | 225 |
| `acl-top+price-50-200` | 22 | 6 | 212 | 204 | 237 | 297 |

Findings:

- **STANDARD is cheap on every filtered regime — 6-10 ms p50.** Even with cross-fields multi_match (boost 99), syntheticKeywords, the full identifier-match fan-out, and DFS\_QUERY\_THEN\_FETCH's extra round-trip, ACL pruning leaves BM25 with little work to do. The post-filter narrowing on top of ACL doesn't materially shift it. Without ACL (`unfiltered`), STANDARD jumps to 11 ms p50 — still the fastest of all six variants — but its tail widens further under broad-corpus load (see saturation note).
...
> **Correction — measured 2026-05-18 (v5 sweep, `profile:true` + `_nodes/hot_threads`; see `next-gen` `article/search/query/BENCHMARK_v5_2026-05-18.md`).** The above was an a-priori extrapolation; direct measurement at ~7× corpus (v5: 920 M docs / ~43 M vectors, vs v4 ~6 M) revises several claims. **Supersedes:** "HNSW … scales gracefully" (the O(log N) row); "the narrow-filter short-circuit … switches to **exact brute force** on survivors"; "the kNN side is the *fast* path"; "**Mitigations** … address the structural filter cost — **not the vector side**"; and "**HNSW at 150M articles is fine.**"
>
> - **There is no exact/brute-force code path.** The filtered marketplace kNN always runs *approximate*-HNSW: `DiversifyingChildrenFloatKnnVectorQuery.approximateSearch` → `Lucene99HnswVectorsReader.search`. The "narrow-filter short-circuit / exact brute force" model is wrong.
> - **HNSW is not filter-aware over the nested parent→child block-join.** Under a low-selectivity scope filter the graph walk over-explores to collect `num_candidates` filter-accepted children, touching **≈ the entire filtered set** (`vector_operations_count ≈ passing-set`; hot frame `OffHeapFloatVectorValues.ordToDoc`). So the corrected scaling law for the *filtered* path is **≈ O(scope passing-set / shard), corpus-independent** — not O(log N). Empirically v5 ≈ v4 at warm steady state *because their scope passing-sets were similar (~1.5 M)*, **not** because the 7× graph is cheap to traverse.
> - **The kNN side is therefore NOT the fast path under realistic ACL scope.** It is a *second* passing-set-bound cost alongside the nested-filter *evaluation* — comparable order, not negligible. True sublinear HNSW occurs only **unfiltered** (~2.8 % of vectors scanned) or under a **hyper-selective** filter.
> - **Mitigations (1)–(2) do not fix this.** Denormalizing `catalogVersionIds`/price scope to top-level `terms` removes the filter *evaluation* cost, but the flattened filter still yields the same large parent passing-set the filter-unaware graph must over-explore (measured: denorm "wide" regime still ≈ O(1.5 M), ~250 ms). A **vector-side mitigation is now required**, e.g.: filter-aware ANN (ACORN-style filtered HNSW), pre-scoped per-ACL vector subindices, hard `num_candidates` ceilings tied to selectivity, or accepting exact scan only once the scope has been denorm-narrowed below a threshold.
>
> Revised framing: **at realistic ACL scope, *both* the nested-filter evaluation and the vector search itself are passing-set-bound; HNSW does not absorb corpus growth on the filtered path.** "HNSW at 150M is fine" holds only for the unfiltered case, which production never issues.
...
- `k` and `num_candidates` both **5000** — validated against the eval set in §2.1.4.3. This is the only single value that holds recall@10 ≥ 0.98 across every realistic filter regime, including price-range filtering where everything below 5000 leaves meaningful recall on the table. Under broad ACL (the common case), p50 ≈ 27 ms; under tighter facets, ES short-circuits to an exact search and the higher numC costs nothing. The legacy 10000 is overkill: ~5 ms more p50 for +1 recall point. See §2.1.4.3 for the conditional-numC alternative if broad-ACL queries dominate and a tighter latency SLO is binding.
...
- **`rank_window_size`-equivalent = 5000** for each leg. Validated in §2.1.4.3. Below 5000, the kNN leg leaves measurable recall on the table under tight filter regimes (notably price ranges).
...
**The escape valve is filter selectivity.** When the filter list narrows the passing population below `num_candidates`, ES short-circuits to a brute-force scan on the survivors (§2.1.4.3 line 620). Total hits and facet counts become exact again because the candidate window now covers the entire matching set. Whether a query lands in "bounded" or "exact" regime is a property of the filter, not the query.
