...
Copied from `src/embedding_train/text.py` (per the "copy, don't share"
decision in the plan). The serving path must produce byte-identical
rendered text to what the training pipeline produced; any drift here
silently corrupts cached embeddings. Pair changes with a HASH_VERSION
bump in `hashing.py`.
"""
