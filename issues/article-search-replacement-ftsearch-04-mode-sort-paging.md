...
**Legacy reference** (next-gen): sort options in `article/search/query/src/main/java/com/simplesystem/nextgen/article/search/query/infrastructure/search/SearchArticleQueryBuilder.java:267-307` — only `name` (asc/desc) and `price` (asc/desc) supported, default `_score,desc`. `pageSize` cap 500 from `application.yml:22`. `trackTotalHits(true)` is uncapped in legacy.
...
- **Sorting** (per §4.2 table — legacy supports only `name` and `price`; everything else is a deviation):
