...
- **Phase 4 — operational + reindex**: F7 ‖ A5 ‖ I2 ‖ I3. F9 lands here as the topology pivot (refines F1/F3/I1, unblocks F8 and the F4 sort-by-price browse path); F8 follows F9 because its envelope columns split between the two collections. Both pair with the next I3 alias swing.
...
- **Article-level dedup topology** (planned, not yet started — F9) — measured embedded-field-hash dedup ratio of ~3.33× on production data implies ~50 GB HNSW RAM and ~31 hours of TEI bulk-reindex time saved per cycle at the 159M-row projected scale. Decision: split storage into `articles_v{N}` (deduplicated by embedded-field hash, holds the vector + BM25 codes + article-level scalars + per-currency price envelope) and `offers_v{N}` (per-offer scalars, `article_hash` join key, no vectors). Search routes through one of two paths (vector-first or filter-first with bounded probe at 25k hashes). Folds today's `offers_codes` BM25 into `articles_v{N}.sparse_codes`. Refines F1/F3/F6/I1 without changing the ftsearch wire contract; F8's envelope columns split across the two collections; sort-by-price browse (no query) gets a no-ANN ordered-query path on the article-level envelope. Canonical design lives in `article-search-replacement-ftsearch-09-article-dedup.md`. Spec §2.3 captures the related relevance-pool bound on non-relevance sorts (independent decision; needed regardless of dedup).
