...
- New collection schema with all of §7:
  ...
  - retain `name`, `manufacturerName`, `ean`, `article_number`, `category_l1..l5`, `offer_embedding`. Per-offer `catalog_version_id` is a singular VARCHAR (post-F9 correction; one row per source mongo offer on `offers_v{N}`); the article-side `catalog_version_ids ARRAY` envelope on `articles_v{N}` lands as F10.
